...
exports.getHintUsage = function(pin, pinDate, uQuery, callback){
    ...
    console.log("update="+JSON.stringify(uQuery));

    var q = {};
    q.pin = pin;
    q.date = {$gte:startDate,$lte:endDate};

    dataStore.con.collection('reports').findOneAndUpdate(q,uQuery,null,callback);
...
exports.getReportsWaitingForEmail = function(callback){
    ...
    dataStore.con.collection('reports').find({sent:null},{fields:{pin:1, date:1}}).toArray(callback);
