...
exports.logHintLogin = function(usageDate, pin, pinId){
    var doc = {pin:pin, pinId:pinId.toString(), date:usageDate, usedHints:[]};
...
exports.deletePIN = function(pinId, callback) {
    ...
    var id = dataStore.client.ObjectID(pinId);
...
exports.getHintUsage = function(pin,pinDate){
    console.log('getHintUsage');
    // find out id for PIN/date
    // get "login-ok" records for specified date & PIN
    var startDate = new Date(pinDate).setHours(0,0,0).toISOString();
    var endDate = new Date(pinDate).setHours(23,59,59).toISOString();
    console.log("start="+startDate);
    console.log("end="+endDate);
    var q = {};
    q.pin = pin;
    q.event = "login-ok";
    q.date = {$gte:{$date:startDate},$lte:{$date:endDate}};
    ...
    /*
     {
        ...
        "event": "login",
        "date": {
            "$gte": {"$date": "2017-04-18T00:00:00.000Z"}
            "$lte": {"$date": "2017-04-18T23:59:59.999"},
        }
     }
     */
    dataStore.con.collection('log').find(q).toArray(function(err,docs){
        console.log(JSON.stringify(docs));
        if (!err){
            console.log("Found "+docs.length+" logins found on "+pinDate);
            var d = docs[0];
            console.log("Using login having id="+d._id);
        }
    });
